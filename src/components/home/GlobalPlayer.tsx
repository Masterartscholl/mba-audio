...
export const GlobalPlayer: React.FC = () => {
    ...
    return (
        <div className="fixed bottom-0 left-0 right-0 bg-app-bg/95 backdrop-blur-xl border-t border-app-border h-24 z-[100] px-8 flex items-center justify-between animate-in slide-in-from-bottom duration-500">
            {/* Track Info */}
            <div className="flex items-center gap-4 w-1/4">
                <div className="w-14 h-14 rounded-xl bg-app-card overflow-hidden shadow-2xl flex-shrink-0">
                    <img
                ...
                <div className="min-w-0">
                    <h4 className="text-app-text font-black text-sm uppercase truncate tracking-tight">{currentTrack.title}</h4>
                    <p className="text-app-text-muted text-[10px] font-bold uppercase truncate tracking-widest leading-none mt-1">{currentTrack.artist_name || 'Unknown Artist'}</p>
                </div>
                <button
                    onClick={handleFavorite}
                    className={`ml-2 transition-colors ${fav ? 'text-app-primary' : 'text-app-text-muted hover:text-app-primary'}`}
